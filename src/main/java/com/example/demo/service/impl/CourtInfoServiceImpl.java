...
@Service
public class CourtInfoServiceImpl implements CourtOpenInfoService{
	@Autowired
...
//			先通过"-"分割字符串，再去除时间数据中的冒号，最后转化为int类型
			...
			if (courtOpenInfo.getCourtOpenItemId() == 1) {
			...
			if (courtOpenInfo.getCourtOpenItemId() == 2) {
				if (openTime > currentTimeInt) {
					tableTennisNotStart++;
				}
				if (endTime < currentTimeInt) {
			...
			if (courtOpenInfo.getCourtOpenItemId() == 3) {
				if (openTime > currentTimeInt) {
					badmintonNotStart++;
				}
				if (endTime < currentTimeInt) {
			...
			if (courtOpenInfo.getCourtOpenItemId() == 4) {
				if (openTime > currentTimeInt) {
					volleyballNotStart++;
				}
				if (endTime < currentTimeInt) {
